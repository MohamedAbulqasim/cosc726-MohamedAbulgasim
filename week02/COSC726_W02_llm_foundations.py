...
You have implemented THREE functions:
...
def prepare_context(messages: list[str], context_limit: int, reserved_output: int,
    ...
    # Calculate total token cost for all messages
    total_cost = sum(count_tokens(msg, tokenizer) for msg in messages)

    if strategy == "reject":
        if total_cost > input_budget:
            return ContextPlan(
                kept=[],
                dropped=[],
                rejected=True,
                reason=f"Total token cost {total_cost} exceeds available input budget {input_budget}."
            )
        else:
            return ContextPlan(
                kept=list(messages),
        ...
        if not messages:
            return ContextPlan(kept=[], dropped=[], rejected=False, reason="")

        # System message (messages[0]) must always be kept
        system_msg = messages[0]
        history_msgs = list(messages[1:])
        dropped_msgs = []

        # Iteratively check and drop from the oldest non-system message
        while history_msgs:
            current_kept = [system_msg] + history_msgs
            current_cost = sum(count_tokens(msg, tokenizer) for msg in current_kept)
            if current_cost <= input_budget:
                break
            # Drop the oldest of the conversation turns
            dropped_msgs.append(history_msgs.pop(0))

        # Final check if even just the system message exceeds the budget
        final_kept = [system_msg] + history_msgs
        final_cost = sum(count_tokens(msg, tokenizer) for msg in final_kept)
        if final_cost > input_budget:
            return ContextPlan(
                kept=[],
                dropped=dropped_msgs + history_msgs,
                rejected=True,
                reason=f"Even system message alone exceeds available input budget {input_budget}."
            )
...
def sample_next(distribution: dict[str, float], temperature: float,
    ...
    # Sort items to ensure deterministic tie-breaking (alphabetically first for ties)
    tokens = sorted(distribution.keys())

    if temperature <= 0:
        # Greedy / argmax with alphabetical tie-breaking
        best_token = max(tokens, key=lambda t: (distribution[t], [-ord(c) for c in t]))
        return best_token

    # Temperature > 0: rescale by temperature
    scaled_weights = {}
    for t in tokens:
        prob = distribution[t]
        if prob <= 0:
            scaled_weights[t] = 0.0
        else:
            scaled_weights[t] = math.pow(prob, 1.0 / temperature)
    ...
    normalized_probs = {t: scaled_weights[t] / total_weight for t in tokens}

    # Cumulative walk sampling using rng.random()
    r = rng.random()
    cumulative = 0.0
    for t in tokens:
        cumulative += normalized_probs[t]
        if r <= cumulative:
            return t
